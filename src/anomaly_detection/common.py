...
class FaissNN(object):
    ...
    def run(
        ...
        if index_features is None:
            return self.search_index.search(query_features, n_nearest_neighbours) # n_nearest_neighbours==1
...
class NearestNeighbourScorer(object):
    def __init__(self, n_nearest_neighbours: int, nn_method=FaissNN(False, 4)) -> None:
        ...
        self.timeserielevel_nn = lambda query: self.nn_method.run(
    ...
    def fit(self, detection_features: List[np.ndarray]) -> None:
        ...
        Args:
            detection_features: [list of np.arrays]
                ...
                np.arrays for all training timeseries corresponding to respective
                features VECTORS (or maps, but will be resized) produced by
                some backbone network which should be used for timeserie-level
                anomaly detection.
        """
    ...
    def predict(
        ...
        Searches for nearest neighbours of test timeseries in all
        support training timeseries.
        ...
        """
        query_features = self.feature_merger.merge( # (N x W x H, 1024)
            query_features,
        )
        query_distances, query_nns = self.timeserielevel_nn(query_features)

        anomaly_scores = np.mean(query_distances, axis=-1) # wasnt the corepatch paper using max instead of mean?
